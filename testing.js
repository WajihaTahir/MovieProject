...
document.addEventListener('DOMContentLoaded', function () {
  ...
  checkboxes.forEach(function (checkbox) {
      checkbox.addEventListener('change', displaySelectedData);
  });
});

function displaySelectedData(){
    // Get the checked checkbox
    const selectedCheckbox = document.querySelector('.type-checkbox:checked');
     // Check if at least one checkbox is selected
     if (!selectedCheckbox) {
      alert('Please select a type.');
      return;
  }
  displayMovieList(selectedCheckbox)
}
const typeParam = selectedCheckbox.value;
